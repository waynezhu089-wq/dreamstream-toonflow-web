    ...
    <section class="supervisor-inspector" @wheel.stop @mousedown.stop @pointerdown.stop>
      ...
      <template v-if="target && gate">
        ...
        <section class="ai-section">
          <h4>AI Supervisor</h4>
          <p>AI 审查只提供当前版本的建议；人工 PASS / REVISE 始终优先。</p>
          <p v-if="aiError" class="error" role="alert">{{ aiError }}</p>
          <p v-if="aiLoading">正在读取当前 Supervisor Skill…</p>
        ...
        <article v-for="row in history" :key="row.reviewId" class="history">
          <strong>{{ row.decision }} · {{ row.status }}<span v-if="row.effective"> · 当前生效</span></strong> · {{ row.source }} · {{ row.actorDisplayName || (row.source === 'AI' ? 'AI Supervisor' : '审核人未知') }} · {{ new Date(Number(row.createdAt)).toLocaleString() }}
          <p>{{ row.summary }}</p>
          <p v-if="row.source === 'AI'">Skill {{ row.supervisorSkillId }} @ v{{ row.supervisorSkillVersion }} · 模型 {{ row.modelReference }} · 策略 {{ row.supervisorResolutionHash?.slice(0, 12) }}</p>
          <p v-if="row.staleReason" class="warning">过期原因：{{ row.staleReason }}</p>
...
async function loadAi(current: number) {
  aiLoading.value = true;
  try { const value = await post('ai/context', scope()); if (current === epoch) aiContext.value = value; }
...
.actions,.editor{margin:14px 0}.editor,.history,.issue,.ai-section{border:1px solid var(--td-component-border);border-radius:6px;padding:12px;margin:8px 0;background:var(--td-bg-color-container)}
